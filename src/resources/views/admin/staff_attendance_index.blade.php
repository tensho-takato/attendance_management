...
<main class="attendance-list">
    ...
    <div class="attendance-list__month-nav">
        <a href="{{ route('admin.staff.attendance.list', ['user' => $user->id, 'month' => $prevMonth]) }}">← 前月</a>

        <div class="attendance-list__month">
            <span>{{ \Carbon\Carbon::createFromFormat('Y-m', $month)->format('Y/m') }}</span>
        </div>

        <a href="{{ route('admin.staff.attendance.list', ['user' => $user->id, 'month' => $nextMonth]) }}">翌月 →</a>
    </div>
        ...
        @foreach($dates as $d)
            @php
                ...
                if ($a) {
                    foreach ($a->breaks as $b) {
                        if ($b->break_start_at && $b->break_end_at) {
                            $breakSeconds += \Carbon\Carbon::parse($b->break_end_at)->diffInSeconds(\Carbon\Carbon::parse($b->break_start_at));
                        }
                    }
                }
                ...
                if ($a && $a->clock_in_at && $a->clock_out_at) {
                    $workSeconds = \Carbon\Carbon::parse($a->clock_out_at)->diffInSeconds(\Carbon\Carbon::parse($a->clock_in_at)) - $breakSeconds;
