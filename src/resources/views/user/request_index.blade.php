...
<main class="request-list">
    ...
    <div class="request-tabs">
        <a href="{{ route('scr.list', ['tab' => 'pending']) }}"
           class="{{ ($tab ?? 'pending') === 'pending' ? 'is-active' : '' }}">
            承認待ち
        </a>
        <a href="{{ route('scr.list', ['tab' => 'approved']) }}"
    ...
    <table class="request-table">
        ...
        @forelse($requests as $r)
            <tr>
                <td>{{ $r->status === 0 ? '承認待ち' : '承認済み' }}</td>
                <td>{{ $r->user?->name ?? '' }}</td>
                <td>{{ \Carbon\Carbon::parse($r->requested_work_date)->format('Y/m/d') }}</td>
                <td>{{ $r->note }}</td>
                <td>{{ $r->created_at?->format('Y/m/d') }}</td>
                <td><a href="{{ route('attendance.detail', ['id' => $r->attendance_id]) }}">詳細</a></td>
            </tr>
        @empty
